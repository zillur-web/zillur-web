...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (!in_array($ext, $allow_format)) {
		...
		header('location:general-settings-edit.php');
	}
		...
		$insart = "UPDATE general_settings SET website_title = '$title', website_subtitle = '$subtitle', logo = '$new_ext' WHERE id = '$id'";
		$query = mysqli_query($db, $insart);

		if ($query) {
			$_SESSION['Success'] = 'General Settings Update Successfully';
